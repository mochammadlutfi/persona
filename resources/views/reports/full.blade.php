            ...
            <tbody>
                @foreach ($data as $a)
                    <tr>
                        ...
                        <td>{{ $a->training->lokasi }}</td>
                        <td>Rp {{ number_format($a->training->harga,0,',','.') }}</td>
                        <td>Rp {{ number_format($a->jumlah,0,',','.') }}</td>
                        <td>{{ \Carbon\Carbon::parse($a->tgl)->translatedFormat('d M Y') }}</td>
                        <td>
                            {{ \Carbon\Carbon::parse($a->training->tgl_mulai)->translatedFormat('d M Y')}}
                            @if($a->training->tgl_selesai)
                            - {{ \Carbon\Carbon::parse($a->training->tgl_selesai)->translatedFormat('d M Y')}}
                            @endif
                        </td>
                        <td>{{ $a->status }}</td>
                    </tr>
